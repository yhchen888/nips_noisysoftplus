...
What's more, the activation function devoted \textbf{TRADES?} the modelling accuracy for computational simplicity.


Even though these activation functions have been successfully used to train ANNs to transfer their weights to SNNs, they do not solve theg the neural response activity of LIF neurons. % with abstract activation functions.
...
Training only employed Noisy Softplus units for all the convolution, average sampling, and fully-connected neurons. % use Noisy Softplus function with no bias.
...
This training procedure consists of two simple stages: first, estimate PAF parameters using Noisy Softplus, and second, use a PAF version of conventional activation functions in ANN training. % can be generalised to activation units other than Noisy Softplus.
