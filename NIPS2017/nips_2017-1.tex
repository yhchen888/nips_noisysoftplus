...
We call these activation functions `abstract' referring to the ones without physical units which used in the ANNs, and select them for LIF modelling due to their simplicity and generalised training in ANNs.
...
Then the second problem appeared to map the abstract activation functions to actual physical units: current in $nA$ and firing rates in $Hz$.
...
\label{sec:af_model}
The inputs of the Noisy Softplus, $x$ and $\sigma$, are obtained from physical variables as stated in Equation~\ref{equ:distr}, thus already in physical unit: $nA$.
Therefore, linearly scaling up the activation function by a factor~$S~Hz$ can approximate the output firing rate $\lambda_{out}$ of an LIF neuron in $Hz$:
\begin{equation}
...
A trained $5\times5$ kernel was convolved with these input digits, and the convolved output of the feature map, the output firing rate was recorded during a real-time SNN simulation on NEST, and compared to the modelled activations of Equation~\ref{equ:PAF} in ANNs.
...
It is useful to compare with existing SNN training methods in Table~ where we order them on the computation complexity (descending).
